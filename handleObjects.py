...
# file read
read_filename = 'c:\\tmp\\excel_objects.xlsx' 
excel = gencache.EnsureDispatch('Excel.Application')
wb = excel.Workbooks.Open(read_filename)  #wb = excel.Workbooks.open(r"c:\tmp\excel_objects.xlsx")
#excel.Visible = True
...
def set_header_data(worksheet, headrow):
  ...
  worksheet.Cells(headrow,2).Value  = '설문대상자'
  worksheet.Cells(headrow,3).Value  = '작성자'
  worksheet.Cells(headrow,4).Value  = '작성자이메일'
  worksheet.Cells(headrow,5).Value  = 'q1'
  worksheet.Cells(headrow,6).Value  = 'q2'
  worksheet.Cells(headrow,7).Value  = 'q3-1'
  worksheet.Cells(headrow,8).Value  = 'q3-2'
  worksheet.Cells(headrow,9).Value  = 'q3-3'
  worksheet.Cells(headrow,10).Value  = 'q3-4'
  worksheet.Cells(headrow,11).Value  = 'q4'
  worksheet.Cells(headrow,12).Value = 'q5'
  worksheet.Cells(headrow,13).Value = 'q6'
  worksheet.Cells(headrow,14).Value = 'q7'
  worksheet.Cells(headrow,15).Value = 'q8'

# write answers list items
def merge_list(inputs, worksheet, startrow):
  print("merge List count : %d" % len(inputs))
  write_row_cursor = startrow
  for input in inputs:
    print(input)
    worksheet.Cells(write_row_cursor,1).Value = write_row_cursor - 1
    worksheet.Cells(write_row_cursor,2).Value = input['surveyname']
    worksheet.Cells(write_row_cursor,3).Value = input['writer']
    worksheet.Cells(write_row_cursor,4).Value = input['wemail']
    worksheet.Cells(write_row_cursor,5).Value = input['q1']
    worksheet.Cells(write_row_cursor,6).Value = input['q2']
    worksheet.Cells(write_row_cursor,7).Value = input['q3-1']
    worksheet.Cells(write_row_cursor,8).Value = input['q3-2']
    worksheet.Cells(write_row_cursor,9).Value = input['q3-3']
    worksheet.Cells(write_row_cursor,10).Value = input['q3-4']
    worksheet.Cells(write_row_cursor,11).Value = input['q4']
    worksheet.Cells(write_row_cursor,12).Value = input['q5']
    worksheet.Cells(write_row_cursor,13).Value = input['q6']
    worksheet.Cells(write_row_cursor,14).Value = input['q7']
    worksheet.Cells(write_row_cursor,15).Value = input['q8']
    write_row_cursor += 1
...
if os.path.exists(master_fname) : # exist file
  ...
  mws = mwb.Worksheets('AnswerMaster') 

  # find the next row cursor for writing
  write_row_cursor = find_next_row_cursor(mws)

  # write list data to merge file
  write_row_cursor = merge_list(answers, mws, write_row_cursor)
